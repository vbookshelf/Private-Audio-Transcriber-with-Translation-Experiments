...
# License: MIT
# Version: 1.6 (Removable Languages)
#----------------------
...
SUPPORTED_LANG_FILE = "supported-languages.json"
DEFAULT_LANGUAGES = [
    "English - en-US",
	"English - en-GB",
    "French - fr-FR",
...
<body>
    ...
    <main class="right-col">
        <div class="right-header">
            <span class="header-note">Privately Transcribe, Translate & Edit</span>
    ...
    <script>
        ...
        fetch('/get_supported_languages')
            .then(res => res.json())
            .then(data => { supportedLanguages = data; });
        ...
        langSearch.addEventListener('input', (e) => {
            ...
            // Modified algorithm: Search only from the start of the language name
            const filtered = supportedLanguages.filter(l => 
                l.language.toLowerCase().startsWith(val)
            ).slice(0, 10);
            ...
            if (filtered.length > 0) {
                filtered.forEach(l => {
                    ...
                    const langStr = `${l.language} - ${l.code}`;
                    div.textContent = langStr;
                    div.onclick = () => addLanguage(langStr);
...
def translate():
    ...
    target_raw = data.get('language') # e.g. "Spanish - es-ES"
    source_lang_code = data.get('source_lang_code', 'auto')
    
    # Parse target language string
    if " - " in target_raw:
        target_lang, tgt_lang_code = target_raw.split(" - ", 1)
    else:
        target_lang, tgt_lang_code = target_raw, target_raw

    # Official TranslateGemma Prompt Implementation
    prompt = (
        f"You are a professional {source_lang_code} to {target_lang} ({tgt_lang_code}) translator. "
        f"Your goal is to accurately convey the meaning and nuances of the original {source_lang_code} text "
        ...
        f"Please translate the following {source_lang_code} text into {target_lang}:\n\n\n{text_to_translate}"
    )

    ollama_api_url = "http://localhost:11434/api/generate"
    payload = {"model": "translategemma:12b", "prompt": prompt, "stream": False}
    
    try:
        response = requests.post(ollama_api_url, json=payload, timeout=120) 
        response.raise_for_status() 
        return jsonify({"translation": response.json().get('response', '').strip()})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
