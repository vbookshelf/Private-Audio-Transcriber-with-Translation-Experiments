...
echo "============================================"
echo ""
echo "   [PRIVACY] Your audio is transcribed and deleted locally."
echo "   [INFO] This window shows exactly what the app is doing."
echo "   [INFO] Close it to shut down the app. (Ctrl+C)"
echo ""
...
# --- Download HuggingFace Model (only if missing) ---
MODEL_DIR="./models/tiny-aya-global-8bit-mlx"
if [ -d "$MODEL_DIR" ] && [ -n "$(ls -A "$MODEL_DIR" 2>/dev/null)" ]; then
    echo "[INFO] Model already exists at $MODEL_DIR - skipping download"
else
    echo "[INFO] Downloading model: mlx-community/tiny-aya-global-8bit-mlx"
    uvx --from huggingface-hub hf download mlx-community/tiny-aya-global-8bit-mlx --local-dir "$MODEL_DIR" || {
        echo "[ERROR] Failed to download model"
        exit 1
    }
fi
